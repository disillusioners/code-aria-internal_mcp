...
func TestCheckpointManagerEdgeCases(t *testing.T) {
	...
	// Test creating checkpoint with special characters in name
	t.Run("SpecialCharactersInName", func(t *testing.T) {
		...
		specialName := "checkpoint-with-special-chars-@#$%^&*()_+-=[]{}|;':\",./<>?"
		checkpoint, err := manager.CreateCheckpoint(specialName, "Testing special characters")
		if err != nil {
			t.Fatalf("Failed to create checkpoint with special characters: %v", err)
		}

		if checkpoint.Name != specialName {
			t.Errorf("Expected name with special chars, got: %s", checkpoint.Name)
		}
	})
	...
	t.Run("LongDescription", func(t *testing.T) {
		...
		checkpoint, err := manager.CreateCheckpoint("test", longDesc)
		if err != nil {
			t.Fatalf("Failed to create checkpoint with long description: %v", err)
		}

		if checkpoint.Description != longDesc {
			t.Errorf("Description length mismatch. Expected %d, got %d", len(longDesc), len(checkpoint.Description))
		}
	})
	...
	t.Run("BinaryFiles", func(t *testing.T) {
		...
		checkpoint, err := manager.CreateCheckpoint("binary-test", "Testing binary files")
		if err != nil {
			t.Fatalf("Failed to create checkpoint with binary file: %v", err)
		}

		// Verify binary file was copied correctly
		checkpointPath := filepath.Join(manager.checkpointDir, checkpoint.ID)
		copiedFile := filepath.Join(checkpointPath, "binary.bin")
	...
	t.Run("NestedDirectories", func(t *testing.T) {
		...
		checkpoint, err := manager.CreateCheckpoint("nested-test", "Testing nested directories")
		if err != nil {
			t.Fatalf("Failed to create checkpoint with nested directories: %v", err)
		}

		// Verify nested structure was preserved
		checkpointPath := filepath.Join(manager.checkpointDir, checkpoint.ID)
		copiedNestedFile := filepath.Join(checkpointPath, "dir1", "dir2", "dir3", "nested.txt")
	...
	t.Run("FilePermissions", func(t *testing.T) {
		...
		checkpoint, err := manager.CreateCheckpoint("perm-test", "Testing file permissions")
		if err != nil {
		...
		checkpointPath := filepath.Join(manager.checkpointDir, checkpoint.ID)
		for _, file := range files {
			copiedFile := filepath.Join(checkpointPath, file.name)
...
func TestCheckpointConcurrentAccess(t *testing.T) {
	...
	// Create multiple files for concurrent checkpoint creation
	for i := 0; i < 10; i++ {
	...
	for i := 0; i < 10; i++ {
		go func(index int) {
			_, err := manager.CreateCheckpoint(
				"concurrent-"+string(rune('A'+index)),
				"Concurrent checkpoint "+string(rune('A'+index)),
			)
			resultChan <- err
		}(i)
	}
	...
	for i := 0; i < 10; i++ {
		if err := <-resultChan; err != nil {
			t.Errorf("Concurrent checkpoint creation failed: %v", err)
		}
	}
	...
	if len(checkpoints) != 10 {
		t.Errorf("Expected 10 checkpoints, got %d", len(checkpoints))
	}
}

func TestCheckpointRestoreWithDeletions(t *testing.T) {
	...
	// Create checkpoint with original files
	checkpoint, err := manager.CreateCheckpoint("original", "Original state")
	if err != nil {
		t.Fatalf("Failed to create checkpoint: %v", err)
	}
...
func TestCheckpointTimestampConsistency(t *testing.T) {
	...
	// Record time before creating checkpoint
	before := time.Now()

	checkpoint, err := manager.CreateCheckpoint("timestamp-test", "Testing timestamps")
	if err != nil {
		t.Fatalf("Failed to create checkpoint: %v", err)
	}

	// Record time after creating checkpoint
	after := time.Now()

	// Parse checkpoint timestamp
	checkpointTime, err := time.Parse(time.RFC3339, checkpoint.Timestamp)
	if err != nil {
		t.Fatalf("Failed to parse checkpoint timestamp: %v", err)
	}

	// Verify timestamp is within expected range
	if checkpointTime.Before(before) {
		t.Error("Checkpoint timestamp is before creation time")
	}

	if checkpointTime.After(after) {
		t.Error("Checkpoint timestamp is after creation time")
	}
}

func TestCheckpointWithSymlinks(t *testing.T) {
	...
	checkpoint, err := manager.CreateCheckpoint("symlink-test", "Testing symlinks")
	if err != nil {
		t.Fatalf("Failed to create checkpoint with symlinks: %v", err)
	}

	// Verify symlink was copied as a file (not as a symlink)
	checkpointPath := filepath.Join(manager.checkpointDir, checkpoint.ID)
	copiedSymlink := filepath.Join(checkpointPath, "symlink.txt")
...
func TestCheckpointWithIgnoredFiles(t *testing.T) {
...
`
	...
	checkpoint, err := manager.CreateCheckpoint("ignored-test", "Testing ignored files")
	if err != nil {
		t.Fatalf("Failed to create checkpoint: %v", err)
	}

	// Verify only tracked files are in checkpoint
	if len(checkpoint.Files) != 1 || checkpoint.Files[0] != "tracked.txt" {
		t.Errorf("Expected only tracked.txt in checkpoint, got: %v", checkpoint.Files)
	}

	// Modify tracked file and create another checkpoint
	err = os.WriteFile(trackedFile, []byte("modified tracked content"), 0644)
	if err != nil {
		t.Fatalf("Failed to modify tracked file: %v", err)
	}

	checkpoint2, err := manager.CreateCheckpoint("ignored-test-2", "Testing with modifications")
	if err != nil {
		t.Fatalf("Failed to create second checkpoint: %v", err)
	}

	// Verify still only tracked files are included
	if len(checkpoint2.Files) != 1 || checkpoint2.Files[0] != "tracked.txt" {
		t.Errorf("Expected only tracked.txt in second checkpoint, got: %v", checkpoint2.Files)
	}
}
