...
func TestToolGetSavepointInfo(t *testing.T) {
	tempDir, err := os.MkdirTemp("", "savepoint-tool-test")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	...
	if err != nil {
		t.Fatalf("Failed to get savepoint info: %v", err)
	}
	...
	if response["savepoint_id"] != savepoint1.ID {
		t.Errorf("Expected savepoint_id %s, got %v", savepoint1.ID, response["savepoint_id"])
	}
	...
	if response["description"] != "First savepoint" {
		t.Errorf("Expected description 'First savepoint', got %v", response["description"])
	}
	...
	// Test getting info for non-existent savepoint
	args = map[string]interface{}{
		"savepoint_id": "nonexistent",
	}

	_, err = toolGetSavepointInfo(args)
	if err == nil {
		t.Error("Expected error when getting info for non-existent savepoint")
	}
}
