...
// TestSuiteRunner helps run comprehensive test suites for the savepoint MCP tool
...
func GetTestFiles(category TestCategory) []string {
	...
	case TestCategoryUnit:
		return []string{
			"savepoint_manager_test.go",
			"savepoint_operations_test.go",
		}
	...
	case TestCategoryEdgeCase:
		return []string{
			"savepoint_manager_edge_test.go",
			"test_utils_example_test.go",
		}
	case TestCategoryPerformance:
		return []string{
			"savepoint_benchmark_test.go",
		}
...
func ValidateTestEnvironment() error {
	...
	if _, err := os.Stat("main.go"); os.IsNotExist(err) {
		return fmt.Errorf("main.go not found. Please run tests from the mcp-savepoints directory")
	}
...
func CleanupTestArtifacts() error {
	...
	artifacts := []string{
		...
		"bench_coverage.out",
		"*.test",
		".mcp-savepoints", // Leftover from failed tests
	}
	...
	for _, pattern := range artifacts {
		...
		for _, match := range matches {
			if strings.Contains(match, ".mcp-savepoints") {
...
func ShowTestSummary() {
	fmt.Println("=== Savepoint MCP Test Suite Summary ===")
	fmt.Println()
